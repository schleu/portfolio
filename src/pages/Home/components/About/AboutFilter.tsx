...
import { FaCheck, FaCheckSquare, FaChevronDown, FaFilter, FaSquare } from 'react-icons/fa'
...
export const AboutFilter = ({ onFilterChange }: AboutFilterProps) => {
    ...
    return (
      <div className="flex gap-2"> 
        ...
        >
          {filter} <FaFilter className={classNames(
          ...
          isOpen ? "max-h-[600px] min-h-fit opacity-100" : "max-h-0 opacity-0"
            ...
            <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                    ...
                    <button className="capitalize text-zinc-400 text-sm" onClick={() => handleSelectAll()}>
                        {stackSelected.length > 0 ? "desmarcar todos" : "selecionar todos"}
                    </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 ml-2">
                    {stackKeys.map((e) => ( 
                        <div key={e}>
                            <button 
                                className={classNames("text-zinc-400 text-xs capitalize flex items-center gap-2")} 
                            ...
                            <div className="flex flex-wrap gap-2 ml-2">
                                {stackMoked[e as keyof typeof stackMoked].map((e) => ( 
                                    <button key={e} className={classNames(
                                        "cursor-pointer capitalize text-start px-4 py-2 text-sm",
                                        "hover:border-primary rounded-md border border-dark-300",
                                        "transition-all duration-500 ease-in-out",
                                        stackSelected.includes(e) ? "text-primary" : "text-dark-100",
                                    )}
