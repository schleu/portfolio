...
import { CustomExercise, Exercise, Workout, WorkoutExercise } from './types';
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8 md:px-8">
        <div className="mb-10 bg-zinc-900/70 backdrop-blur-xl border border-zinc-800 rounded-[32px] p-8 shadow-2xl">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-zinc-800/70 border border-zinc-700 rounded-3xl p-5 text-center min-w-[110px]">
                <div className="text-3xl font-black text-red-400">{allExercises.reduce((acc, item) => acc + item.image.length, 0)}</div>
        ...
        <div className="flex flex-wrap gap-4 mb-10">
          {workouts.map((w) => (
            ...
            >
              <span className="relative z-10">Treino {w.training}</span>
            </button>
          ))}
        </div>

        <ActualWorkout currentWorkout={currentWorkout} />

        {/* <PersonalizeTraining selectedWorkout={selectedWorkout} /> */}
...
interface ActualWorkoutProps{
  currentWorkout:Workout
}

function ActualWorkout({currentWorkout}:ActualWorkoutProps){
  const allExercises: Exercise[] = mockedExercises;
  ...
  useEffect(()=> {
    ...
    setCurrent(0)
    setExercises(currentWorkout.exercises.sort((a,b)=>a.order - b.order))
  

  },[currentWorkout])

  const currentExercie = exercises[current]
  const exer = allExercises.find(e=>e.id === currentExercie.id)

  function nextExercise(){
    setCurrent(i=> i < exercises.length-1 ? i+1 : i)
  }
  ...
  return (
    <div className="w-full gap-8 mb-10">
      <div className="bg-zinc-900/70 backdrop-blur-xl border border-zinc-800 rounded-[32px] p-6 shadow-2xl w-full">
        ...
        <div className="grid grid-cols-3">
          <div className={`flex flex-col items-center`}>
            <p>{allExercises.find(e=>e.id === exercises[current===0 ? 0 :current-1 ].id)?.name}</p>
            <small>{allExercises.find(e=>e.id === exercises[current===0 ? 0 :current-1 ].id)?.muscle}</small>

            <button onClick={previousExercise} disabled={current===0}>
              Previous
            </button>
          </div>
            
          <div
            key={currentExercie.order}
            className={`
              w-full md:w-1/4 min-w-[360px] group bg-gradient-to-r from-zinc-900 to-zinc-800 border border-zinc-700 hover:border-green-500/40 hover:-translate-y-1 rounded-3xl p-5 transition-all duration-300 
            `}
          >
            <div className="flex flex-col gap-4">

              <img src={Array.isArray(exer?.image) ? exer?.image[0] : exer?.image} alt="" className='w-fit h-full max-h-40 rounded-md mx-auto' />
              ...
              <div className='flex flex-col gap-4'>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="bg-zinc-700/70 text-zinc-300 text-xs px-3 py-1.5 rounded-xl font-semibold">
                    <p>
                      Serie: 
                      <span className='ml-2'>
                        {currentExercie.series}
                      </span>
                    </p>
                  </div>
              ...
              <VideoPlayer url={exer?.video+"?origin=http://example.com"} />
              
            </div>
          </div>


          <div className={`flex flex-col items-center ${current === exercises.length-1 ? 'hidden' : ''}`}>
            <p>{allExercises.find(e=>e.id === exercises[current+1].id)?.name}</p>
            <small>{allExercises.find(e=>e.id === exercises[current+1].id)?.muscle}</small>
...
interface PersonalizeTrainingProps{
  selectedWorkout: any
}

function PersonalizeTraining({selectedWorkout}:PersonalizeTrainingProps){
...
function VideoPlayer({url}:{url:string}){
  ...
  const togleHide = ()=>setHide(e=>!e)
  return(
    <div className='w-full overflow-hidden
    '>
      <button onClick={togleHide} className='flex items-center gap-2'>
        <FiVideo />
        Video
      </button>
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${hide?'w-0 h-0 p-0':'w-[320px] h-[180px]'}`} >
        <iframe id="ytplayer" width="320" height="180" src={url}  />
      </div>
    </div>
  )
}